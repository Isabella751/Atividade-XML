...
//server para converter XML em JSON e JSON em XML
import {parseStringPromise, Builder} from "xml2js"

const app = express();
app.use(cors());
app.use(bodyParser.text({type:"application/xml"}))
// informa que o body das requisições sera texto do tipo "application/xml" XML

// Essa função recebe o objeto JavaScript e converte para o XML
const jsonParaXml = (obj, root = "response" )=>{
    // cria um builder configurado para montar XML com cabeçalho padrao UTF-8
  ...
  const novoCliente = {
   id: idCliente++, nome, email
    ...
    res.status(201).type("application/xml").send(jsonParaXml(
      {message: "Cliente cadastrado com sucesso", cliente: novoCliente}))
...
app.get("/clientes", (req, res)=>{
    res.type("application/xml").send(jsonParaXml({clientes: {cliente: clientes}}))
})
...
app.post("/compras", async (req, res) => {
    ...
    if (!cliente) {
        return res.status(404).type(`aplication/xml`).send(jsonParaXml(
...
app.get("/compras", (req, res) => {
    res.type("application/xml").send(jsonParaXml({ compras: { compra: compras } }))
})

const PORTA = 3000
//inicializador do servidor 
app.listen(PORTA , ()=>{
    console.log(`Servidor rodando na em http://localhost:${PORTA}`)
})
